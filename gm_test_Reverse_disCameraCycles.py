...
class test_Reverse_disCameraCycles(tlib.TestExec):
    ...
    def run(self):  
        try:
            ...
            #assuming if camera disable , the display will stay at current mode
            if  self.pc.display.get_Display() != self.pc.display.current_screen:
                self.set_testFail()
...
def test_package(): 
    ...
    usecase_gearPark_dfeault.testname = "usecase_gear_Park2Reverse_disable_Camera_Cycles"
    usecase_gearPark_dfeault.settings["Temp"] = gVars.const.AMBIANT_TEMP
    usecase_gearPark_dfeault.settings["IsDay"] = False
    usecase_gearPark_dfeault.cycles = 5        
    test_Reverse_disCameraCycles(usecase_gearPark_dfeault)
