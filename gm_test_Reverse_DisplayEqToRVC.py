...
class test_Reverse_defaultCheck(tlib.TestExec):
    ...
    def run(self):  
        try:
            ...
            self.pc.disable_guidelines()
            self.pc.change_gear_to_reverse()
            
            
            for ref_image in self.const.RGB_IMAGES:
                self.logger.info("Compare REF Image file: [{}] to [RVC,DISAPLY]".format(ref_image))
                if not self.pc.is_rear_view_camera_displayed_on_screen(ref_image):
                    self.set_testFail()
                    self.logger.info("ASSERT:RVC IMAGE NOT MATCHING REF_IMAGE") 
                    
                self.logger.info("PASS Compare REF Image file: [{}] to [RVC,DISAPLY]".format(ref_image))    
                self.p.PrintLineSeparator_Short()
...
def test_package(): 
    ...
    usecase_gearPark_dfeault.testname = "usecase_gear_Park2Reverse"
    ...
    test_Reverse_defaultCheck(usecase_gearPark_dfeault)
